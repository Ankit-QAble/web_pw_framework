...
test.describe('Login Scenarios - Negative', () => {
  ...
  test('Login with empty password', async ({ logger }) => {
    await logger.step('Attempt login with valid username and empty password', async () => {
      await swagPage.loginWithEmptyPassword(swagData.credentials.standard_user);
    });

    await logger.step('Verify error message for missing password', async () => {
      await swagPage.verifyErrorMessage(swagData.expected.passwordRequired);
    });
  });
});
